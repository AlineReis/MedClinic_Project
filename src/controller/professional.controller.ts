...
export class ProfessionalController {
  ...
  /**
   * Get monthly reports for a professional
   * RN-28: Professionals view own reports, admins view any
   */
  public getMonthlyReports = async (
    req: Request,
    res: Response,
    next: NextFunction,
  ) => {
    try {
      ...
      const filters = { month, year, payment_status: payment_status as any };
      const reports = await this.professionalService.getMonthlyReports(
        professionalId,
        {
          id: req.user.id,
  ...
  public generateMonthlyReport = async (
    ...
    try {
      ...
      const report = await this.professionalService.generateMonthlyReport(
        professionalId,
        Number(month),
        Number(year),
        {
  ...
  /**
   * Mark monthly report as paid (admin only)
   */
  public markReportAsPaid = async (
    req: Request,
    res: Response,
    next: NextFunction,
  ) => {
    try {
      ...
      const report = await this.professionalService.markReportAsPaid(
        reportId,
        {
          role: req.user.role as UserRole,
          clinic_id: req.user.clinic_id,
