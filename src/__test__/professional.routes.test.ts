...
describe("Professional Routes Integration", () => {
  ...
  it("GET /professionals/:id/availability returns slots with default 7 days", async () => {
    ...
    // Test default
    let response = await request(app).get("/professionals/1/availability");
    expect(response.status).toBe(200);
    expect(FakeProfessionalService.lastGetAvailabilityArgs).toEqual([1, 7]);

    // Test explicit
    response = await request(app).get("/professionals/1/availability?days_ahead=3");
    expect(response.status).toBe(200);
    expect(FakeProfessionalService.lastGetAvailabilityArgs).toEqual([1, 3]);
  });
