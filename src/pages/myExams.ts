...
import { listExams, sendExamResultEmail } from "../services/examsService";
...
async function loadMyExams() {
  ...
  if (!response.success || !response.data) {
    if (examsListContainer) {
      examsListContainer.innerHTML = buildErrorState(
        response.error?.message ?? "Erro ao carregar exames.",
      );
    }
    return;
  }
...
function buildExamRow(exam: ExamSummary) {
  ...
  // Email Button (Only if delivered/ready)
  let emailBtn = "";
  if (canView) {
    emailBtn = `
          <button data-email-id="${exam.id}" class="action-btn action-btn--secondary" style="margin-left: 8px;">
...
function attachEmailListeners() {
  document.querySelectorAll("[data-email-id]").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      const el = e.currentTarget as HTMLElement;
      const id = parseInt(el.dataset.emailId!);

      uiStore.addToast("info", "Enviando email...");
      const response = await sendExamResultEmail(id);

      if (response.success) {
        uiStore.addToast("success", "Enviado com sucesso!");
      } else {
