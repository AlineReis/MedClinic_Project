    ...
    chartInstance = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                legend: {
                    position: 'top',
                    labels: { color: '#9ca3af', usePointStyle: true, font: { family: "'Inter', sans-serif", size: 12 } }
                },
                tooltip: {
                ...
                y: {
                    ...
                    ticks: {
                        ...
                        callback: function(value) {
                            if (typeof value === 'number' && value >= 1000) return 'R$ ' + (value / 1000).toFixed(1) + 'k';
                            return 'R$ ' + value;
                        }
                    },
...
function exportToCSV() {
    ...
    const headers = ["Data", "Hora", "Paciente", "Profissional", "Especialidade", "Status", "Valor (R$)"];
    ...
    const rows = filteredAppointments.map(app => {
        ...
        ].map(field => `"${field}"`).join(';'); // Quote fields + Semicolon
    });
...
function printReport() {
    ...
    const total = filteredAppointments.reduce((sum, app) => sum + (app.price || 0), 0);
...
function applyFilters() {
    filteredAppointments = currentAppointments.filter(app => {
        // Search (Patient Name)
        if (filters.search && !app.patient_name.toLowerCase().includes(filters.search)) {
            return false;
        }
        // Date Range
    ...
    filteredAppointments.sort((a, b) => {
        switch (filters.sortBy) {
            ...
            case 'date_desc': return b.date.localeCompare(a.date) || b.time.localeCompare(a.time);
...
function renderTransactionsTable() {
    ...
    if (filteredAppointments.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="text-center" style="padding: 2rem; color: var(--text-secondary);">Nenhuma transação encontrada.</td></tr>';
    ...
    const endIdx = Math.min(startIdx + ITEMS_PER_PAGE, filteredAppointments.length);
    const pageItems = filteredAppointments.slice(startIdx, endIdx);

    // Update Controls
    if (infoSpan) infoSpan.textContent = `Mostrando ${startIdx + 1}-${endIdx} de ${filteredAppointments.length}`;
