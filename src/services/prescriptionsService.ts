...
export async function getPrescription(id: number) {
  const response = await request<
    ...
    | { prescription: PrescriptionDetailApiResponse }
  >(`/prescriptions/${id}`);
  ...
  const rawData: PrescriptionDetailApiResponse =
    "prescription" in response.data
...
export async function createPrescription(payload: CreatePrescriptionPayload) {
  const response = await request<any>("/prescriptions", "POST", {
  ...
  // Handle nested 'prescription' object or flat response
  const prescriptionData = response.data.prescription || response.data;

  return {
    ...response,
    data: mapPrescriptionDetail(prescriptionData),
  };
}
