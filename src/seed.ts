...
async function seedUsers(): Promise<Map<string, number>> {
    ...
    for (const user of users) {
        ...
        const result = await database.run(
            ...
            [user.name, user.email, hashedPassword, user.role, user.cpf || null, user.phone || null]
        );
...
async function seedProfessionalDetails(userIds: Map<string, number>): Promise<void> {
    ...
    for (const detail of details) {
        ...
        await database.run(
            `INSERT INTO professional_details
             ...
             VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`,
            [userId, detail.specialty, detail.registration_number, detail.council, detail.consultation_price, detail.commission_percentage]
        );
    }
...
async function seedAvailabilities(userIds: Map<string, number>): Promise<void> {
    ...
    const availabilities = [
        { email: 'joao@clinica.com', day_of_week: 2, start_time: '09:00', end_time: '12:00' },
        { email: 'joao@clinica.com', day_of_week: 2, start_time: '14:00', end_time: '18:00' },
        { email: 'joao@clinica.com', day_of_week: 4, start_time: '09:00', end_time: '12:00' },
        { email: 'joao@clinica.com', day_of_week: 4, start_time: '14:00', end_time: '18:00' },
        { email: 'ana@clinica.com', day_of_week: 1, start_time: '08:00', end_time: '12:00' },
        { email: 'ana@clinica.com', day_of_week: 3, start_time: '08:00', end_time: '12:00' },
        { email: 'ana@clinica.com', day_of_week: 5, start_time: '08:00', end_time: '12:00' },
        { email: 'carlos@clinica.com', day_of_week: 1, start_time: '07:00', end_time: '11:00' },
        { email: 'carlos@clinica.com', day_of_week: 2, start_time: '07:00', end_time: '11:00' },
        { email: 'carlos@clinica.com', day_of_week: 3, start_time: '07:00', end_time: '11:00' },
        { email: 'carlos@clinica.com', day_of_week: 4, start_time: '07:00', end_time: '11:00' },
        { email: 'carlos@clinica.com', day_of_week: 5, start_time: '07:00', end_time: '11:00' }
    ];
...
async function seedAppointments(userIds: Map<string, number>): Promise<SeededAppointment[]> {
    ...
    const insertedAppointments: { id: number; date: string; professionalEmail: string }[] = [];
    ...
    for (const appointment of appointmentSeeds) {
        ...
        const cancelledById = appointment.cancelled_by_email ? userIds.get(appointment.cancelled_by_email) : null;
...
async function seedExamCatalog(): Promise<Map<string, number>> {
    ...
    const exams = [
        { name: 'Hemograma Completo', type: 'blood', base_price: 80.0, description: 'Análise completa de células sanguíneas' },
        { name: 'Glicemia em Jejum', type: 'blood', base_price: 35.0, description: 'Medição de glicose (requer 8h jejum)' },
        { name: 'Colesterol Total e Frações', type: 'blood', base_price: 95.0, description: 'Perfil lipídico completo' },
        { name: 'TSH e T4 Livre', type: 'blood', base_price: 120.0, description: 'Avaliação da função tireoidiana' },
        { name: 'Ureia e Creatinina', type: 'blood', base_price: 65.0, description: 'Avaliação da função renal' },
        { name: 'Raio-X Tórax', type: 'image', base_price: 120.0, description: 'Radiografia de tórax em PA e perfil' },
        { name: 'Ultrassonografia Abdominal', type: 'image', base_price: 250.0, description: 'Avaliação de órgãos abdominais' },
        { name: 'Eletrocardiograma', type: 'image', base_price: 150.0, description: 'Registro da atividade elétrica do coração' },
        { name: 'Ecocardiograma', type: 'image', base_price: 450.0, description: 'Ultrassom do coração com Doppler' }
    ];
...
async function seedExamRequests(
    ...
    for (const request of requests) {
        const appointment = appointments.find((appt) => appt.date === request.appointmentRefDate);
        ...
        const labTechId = request.labTechEmail ? userIds.get(request.labTechEmail) : null;
...
async function seedPrescriptions(appointments: SeededAppointment[], userIds: Map<string, number>): Promise<void> {
    ...
    for (const prescription of prescriptions) {
        const appointment = appointments.find((appt) => appt.date === prescription.appointmentDate);
        if (!appointment) continue;
...
async function seedTransactions(
    ...
    const transactionSeeds: TransactionSeed[] = [
        { type: 'appointment_payment', referenceId: appointments[0].id, referenceType: 'appointment', payerEmail: 'maria@email.com', amountGross: 350.0, status: 'paid', payment_method: 'credit_card' },
        { type: 'appointment_payment', referenceId: appointments[1].id, referenceType: 'appointment', payerEmail: 'joao.santos@email.com', amountGross: 180.0, status: 'paid', payment_method: 'pix' },
        { type: 'exam_payment', referenceId: examRequests[0], referenceType: 'exam', payerEmail: 'maria@email.com', amountGross: 80.0, status: 'processing', payment_method: 'boleto' }
    ];
    ...
    for (const tx of transactionSeeds) {
        ...
        insertedTransactions.push({ id: result.lastID, payerEmail: tx.payerEmail, type: tx.type, amount: tx.amountGross });
    }
...
async function seedCommissionSplits(
    ...
    for (const transaction of transactions) {
        const professionalEmail = transaction.type === 'appointment_payment' ? 'joao@clinica.com' : 'ana@clinica.com';
...
async function seedRefunds(transactions: { id: number; }[], userIds: Map<string, number>): Promise<void> {
...
async function main(): Promise<void> {
    ...
    try {
        ...
        const examRequests = await seedExamRequests(appointments, userIds, examCatalog);
        await seedPrescriptions(appointments, userIds);

        const transactions = await seedTransactions(appointments, examRequests, userIds);
