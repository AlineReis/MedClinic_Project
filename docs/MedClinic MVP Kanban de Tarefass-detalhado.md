...
**Service / Repository / Controller:** `UserController`, `UserService`
**Arquivos provavelmente afetados:** `src/__tests__/user.routes.test.ts`, `src/controllers/UserController.ts`, `src/services/UserService.ts`
**Descrição:** garantir que o próprio usuário consegue atualizar somente campos permitidos (nome, email) e que campos sensíveis como `role` e `password_hash` permanecem intactos.
**Ponto de apoio:** `docs/DOC_API_ROTAS.md`, `docs/DOCS_REGRAS_NEGOCIO.md`
**Definition of Done:** teste envia payload de nome/email, recebe 200 e valida que outros campos não mudaram; tentativas por outros usuários retornam 403.
**Dependências:** RBAC (3.3.\*), validators (7.2.1).
**Atenção:** confirme o `clinic_id` e use transações para evitar atualizações parciais.
